...
string chooseCategory(string categories[], int numCategories){
	...
	int randNum1 = rand() % numCategories;
	int randNum2 = rand() % numCategories;
	int randNum3 = rand() % numCategories;
	
	if(randNum2 == randNum1){
		randNum2 = (randNum2 + 1) % numCategories;
}
	if(randNum3 == randNum1 || randNum3 == randNum2){
		randNum3 = (randNum3 + 1) % numCategories;
}	
	cout << "Choose a category from one of the following: "<< endl;
	cout <<"1. " << categories[randNum1] << endl;
	cout <<"2. " << categories[randNum2] << endl;
	cout <<"3. " << categories[randNum3] << endl;
	
	int choice;
	cout << "Please enter your choice ";
	cin >> choice;
	if(choice == 1){
		return categories[randNum1];
	} else if(choice == 2){
		return categories[randNum2];
	} else if(choice == 3){
		return categories[randNum3];
	}
...
string choosePuzzle(string category, Puzzle p[], int puzzleCount){
	...
	decryptPuzzle(decryptedPuzzle);
	
	return string(decryptedPuzzle);
}
...
string revealLetter(string decryptedPuzzle) {
    string reveal;
    char consonant1, consonant2, consonant3, vowel, wildCard, choice;

    cout << "Enter three consonants: ";
    cin >> consonant1 >> consonant2 >> consonant3;
    cout << endl;
    ...
    consonant1 = toUpper(consonant1);
    consonant2 = toUpper(consonant2);
    consonant3 = toUpper(consonant3);
    vowel = toUpper(vowel);
    ...
    for (int i = 0; i < decryptedPuzzle.length(); i++) {
        ...
        for (size_t j = 0; j < revealChars.length(); j++) {
            if (c == revealChars[j] || c == consonant1 || c == consonant2 || c == consonant3 || c == vowel || (wildCard == 'Y' && c == choice)) {
                characterFound = true;
                break;
            }
        }
...
int main(){
	...
	int maxcat = 50;
	int maxpuzz = 1300;
	string categories[maxcat];
	Puzzle puzzles[maxpuzz];
    int categoryCount = readCategories(categories);

    
    int puzzleCount = readPuzzles(puzzles,categories, maxpuzz);
	...
	string decryptedPuzzle = randpuzzle;
	rstlne(decryptedPuzzle);;
	cout << "The puzzle is now: " << decryptedPuzzle << endl;
	cout << endl; 
    ...
    for (int i = 1; i <= maxAttempts; i++) {
        ...
        if (strcmp(guesses, answer) == 0) {
            ...
            if (i < maxAttempts) {
                cout << "You have " << maxAttempts - i << " tries left." << endl;
            }
        }
	}
